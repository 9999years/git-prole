...
impl Display for ConvertPlan<'_> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "Converting {} to a worktree repository at {}.\n\
            I'll move the following worktrees to new locations:\n\
            {}",
            NormalPath::try_display_cwd(&self.repo),
            NormalPath::try_display_cwd(&self.destination),
            format_bulleted_list_multiline(self.worktrees.iter().map(|worktree| {
                format!(
                    "{} -> {}",
        ...
        if !self.new_worktrees.is_empty() {
            write!(
                f,
                "\nI'll also create new worktrees for the following branches:\n\
                {}",
...
impl<'a> ConvertPlan<'a> {
    ...
    pub fn new(git: AppGit<'a>, opts: ConvertPlanOpts) -> miette::Result<Self> {
        ...
        tracing::debug!(
            ...
            format_bulleted_list(worktrees.iter().map(|(path, worktree)| {
                format!(
                    "{} → {}",
                    NormalPath::try_display_cwd(path),
                    NormalPath::try_display_cwd(destination.join(&worktree.name)),
                )
            }))
        );
    ...
    fn destination_plan(
        ...
        match main.head {
            WorktreeHead::Detached(_) | WorktreeHead::Branch(_, _) => Ok(main.path.clone()),
            WorktreeHead::Bare => {
                let basename = main
                    .path
                    .file_name()
